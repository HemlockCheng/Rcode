...
library(ggplot2)
library(stats)
x <- c(rnorm(2500, mean = 0, sd = 0.5), rnorm(2500, mean =0, sd = 4))
...
plot <- ggplot() +
  ...
  geom_vline(aes(xintercept=-1.0),linetype="dashed",color="black",size=1.5)+
  geom_vline(aes(xintercept=1.0),linetype="dashed",color="black",size=1.5)+
  geom_vline(aes(xintercept=-0.65),linetype="dashed",color="orange",size=1.5)+
  geom_vline(aes(xintercept=0.65),linetype="dashed",color="orange",size=1.5)+
  scale_x_continuous(breaks = c(-2,-1,-0.65,0,0.65,1,2), labels = c("-2","-1","-0.65","0","-0.65","1","2"))+
  scale_y_continuous(breaks = c(-1,-0.75,-0.5,-0.25,0,0.25,0.5,0.75,1), labels = c("-1","-0.75","-0.5","-0.25","0","0.25","0.5","0.75","1"))+
